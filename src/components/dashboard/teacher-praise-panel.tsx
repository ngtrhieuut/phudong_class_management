    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      ...
      <section>
        ...
        <div className="space-y-4">
          {posts.map((post) => <article key={post.id} className="rounded-[1.5rem] bg-[var(--surface-lowest)] p-5 soft-shadow"><div className="flex items-start justify-between gap-4"><div><h3 className="font-heading text-lg font-bold text-[var(--on-surface)]">{post.title}</h3><p className="mt-1 font-body text-xs text-[var(--on-surface-variant)]">{post.studentNames} · {new Date(post.createdAt).toLocaleDateString("vi-VN")}</p></div><button type="button" disabled={busy} onClick={() => void toggleVisibility(post)} aria-label={post.visibility === "teacher_only" ? "Đăng bài tuyên dương" : "Ẩn bài tuyên dương"} className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--surface-low)] text-[var(--primary)] disabled:opacity-50">{post.visibility === "teacher_only" ? <Eye size={19} /> : <EyeSlash size={19} />}</button></div><p className="mt-4 font-body text-sm leading-7 text-[var(--on-surface-variant)]">{post.body}</p>{post.media?.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{post.media.map((media) => <Image key={media.id} src={`/api/media/${media.id}`} alt={`Media của bài ${post.title}`} width={800} height={600} unoptimized className="max-h-72 w-full rounded-2xl object-cover" />)}</div> : null}<span className="mt-4 inline-flex rounded-full bg-[var(--surface-low)] px-3 py-1 font-heading text-[11px] font-bold text-[var(--on-surface-variant)]">{post.visibility === "teacher_only" ? "Bản nháp" : post.visibility === "class" ? "Cả lớp" : "Phụ huynh liên quan"}</span></article>)}
