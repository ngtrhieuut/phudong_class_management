  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      <section className="rounded-[1.5rem] bg-[var(--surface-lowest)] p-5 soft-shadow">
        ...
        <label className="mt-4 block"><span className="font-heading text-sm font-bold text-[var(--on-surface)]">Ảnh/video đính kèm <span className="font-body text-xs font-normal text-[var(--on-surface-variant)]">(tuỳ chọn, tối đa 50 MB)</span></span><span className="mt-2 flex min-h-12 items-center gap-2 rounded-xl bg-[var(--surface-low)] px-4 font-body text-sm text-[var(--on-surface-variant)]"><FileArrowUp size={20} /><input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" onChange={(event) => selectMedia(event.target.files?.[0])} className="min-w-0 flex-1" /></span>{mediaFile ? <span className="mt-2 block truncate font-body text-xs text-[var(--on-surface-variant)]">Đã chọn: {mediaFile.name}</span> : null}</label>
        <button type="button" disabled={busy} onClick={() => void createPost()} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--primary)] px-5 font-heading text-sm font-bold text-white disabled:opacity-50"><CheckCircle size={19} weight="fill" /> {busy ? uploadProgress !== null ? `Đang upload ${uploadProgress}%...` : "Đang lưu..." : "Đăng tuyên dương"}</button>
