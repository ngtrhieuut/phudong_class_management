...
import { AdminServiceError, executeAdminAction } from "@/lib/admin/service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await executeAdminAction(await request.json(), user.id);
    return NextResponse.json({ data: result }, { status: 201, headers: noStoreHeaders() });
  } catch (error) {
    if (error instanceof AdminServiceError) return NextResponse.json({ error: error.message, code: error.code }, { status: error.code === "FORBIDDEN" ? 403 : error.code === "NOT_FOUND" ? 404 : error.code === "CONFLICT" ? 409 : 422, headers: noStoreHeaders() });
    return NextResponse.json({ error: "Không thể cập nhật quản trị lúc này." }, { status: 500, headers: noStoreHeaders() });
  }
}
