...
import { createClassTask, TaskServiceError } from "@/lib/classroom/task-service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createClassTask(await request.json(), session.user.id);
    ...
    return NextResponse.json({ error: "Không thể tạo nhiệm vụ." }, { status: 500, headers: noStoreHeaders() });
  }
}
