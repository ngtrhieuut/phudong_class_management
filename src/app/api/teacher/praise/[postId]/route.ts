...
import { PraiseServiceError, setPraiseVisibility } from "@/lib/praise/service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ postId: string }> }) {
  ...
  try {
    ...
    const body = await request.json() as { visibility?: "class" | "related_guardians" | "teacher_only" };
    if (!body.visibility) return NextResponse.json({ error: "Visibility không hợp lệ." }, { status: 400, headers: noStoreHeaders() });
    await ensureAppUser({ id: session.user.id, email: session.user.email, name: session.user.name });
    const result = await setPraiseVisibility(postId, body.visibility, session.user.id);
    return NextResponse.json({ data: result }, { headers: noStoreHeaders() });
  } catch (error) {
    if (error instanceof PraiseServiceError) {
      return NextResponse.json({ error: error.message, code: error.code }, { status: error.code === "FORBIDDEN_CLASS_ACCESS" ? 403 : 422, headers: noStoreHeaders() });
    }
    return NextResponse.json({ error: "Không thể cập nhật bài tuyên dương." }, { status: 500, headers: noStoreHeaders() });
  }
}
