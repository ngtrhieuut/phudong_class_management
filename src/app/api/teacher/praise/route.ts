...
import { createPraisePost, PraiseServiceError } from "@/lib/praise/service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createPraisePost(await request.json(), session.user.id);
    ...
    return NextResponse.json({ error: "Không thể tạo bài tuyên dương." }, { status: 500, headers: noStoreHeaders() });
  }
}
