...
import { markAllParentNotificationsRead, markParentNotificationRead, ParentNotificationError } from "@/lib/parent/notification-service";
  ...
  try {
    ...
    const result = body.all === true
      ...
      : typeof body.notificationId === "string"
        ? await markParentNotificationRead(body.notificationId, session.user.id)
        : null;
    if (!result) return NextResponse.json({ error: "Thiếu notificationId hoặc all." }, { status: 422, headers: noStoreHeaders() });
    return NextResponse.json({ data: result }, { headers: noStoreHeaders() });
  } catch (error) {
    if (error instanceof ParentNotificationError) return NextResponse.json({ error: error.message, code: error.code }, { status: error.code === "NOT_FOUND" ? 404 : 422, headers: noStoreHeaders() });
