...
export function TeacherOnboardingForm({ initialOrganizationName, initialSchoolYearName = "2026-2027", initialClassName = "Lớp 1/6", initialGrade = "1" }: { initialOrganizationName: string; initialSchoolYearName?: string; initialClassName?: string; initialGrade?: string }) {
  const router = useRouter();
  const [organizationName, setOrganizationName] = useState(initialOrganizationName);
  const [organizationCode, setOrganizationCode] = useState("");
  const [schoolYearName, setSchoolYearName] = useState(initialSchoolYearName);
  const [startsAt, setStartsAt] = useState("2026-08-01");
  const [endsAt, setEndsAt] = useState("2027-05-31");
  const [className, setClassName] = useState(initialClassName);
  const [grade, setGrade] = useState(initialGrade);
  const [behaviorPreset, setBehaviorPreset] = useState("standard");
  const [levelPreset, setLevelPreset] = useState("standard");
  const [badgePreset, setBadgePreset] = useState("standard");
  const [rewardPreset, setRewardPreset] = useState("standard");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/teacher/onboarding", { method: "POST", headers: { "content-type": "application/json", "idempotency-key": crypto.randomUUID() }, body: JSON.stringify({ organization: { name: organizationName, code: organizationCode || undefined }, schoolYear: { name: schoolYearName, startsAt, endsAt }, classroom: { name: className, grade: Number(grade) }, configuration: { behaviorPreset, levelPreset, badgePreset, rewardPreset }, students: parseStudents() }) });
