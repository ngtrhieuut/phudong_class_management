...
export default async function TeacherRewardsPage({ searchParams }: { searchParams: Promise<{ classId?: string }> }) {
  ...
  if (!board) return <AppShell active="Kho quà" classOptions={classOptions} selectedClassId={classId} classSwitcherPath="/teacher/rewards" teacherName={appUser.displayName}><div className="mx-auto max-w-3xl px-5 py-16 text-center"><h1 className="font-heading text-3xl font-bold text-[var(--primary)]">Chưa có lớp được phân công</h1></div></AppShell>;
  const students = await getClassStudents(session.user.id, board.classContext.id);
  return <AppShell active="Kho quà" classOptions={classOptions} selectedClassId={board.classContext.id} classSwitcherPath="/teacher/rewards" teacherName={appUser.displayName} className={board.classContext.name} schoolYearName={board.classContext.schoolYearName}><div className="mx-auto max-w-[1480px] px-5 py-7 sm:px-8 lg:px-10"><TeacherRewardManager classId={board.classContext.id} students={students.map((student) => ({ id: student.id, name: student.fullName }))} rewards={board.rewards} /></div></AppShell>;
}
