...
export default async function TeacherPraisePage({ searchParams }: { searchParams: Promise<{ classId?: string }> }) {
  ...
  if (!classContext) {
    return <AppShell active="Góc tuyên dương" classOptions={classOptions} selectedClassId={classId} classSwitcherPath="/teacher/praise" teacherName={appUser.displayName}><div className="mx-auto max-w-3xl px-5 py-16 text-center"><h1 className="font-heading text-3xl font-bold text-[var(--primary)]">Chưa có lớp được phân công</h1><p className="mt-3 font-body text-sm text-[var(--on-surface-variant)]">Hãy liên hệ quản trị viên để được gắn lớp.</p></div></AppShell>;
  }
