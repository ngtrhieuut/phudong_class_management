...
import { db } from "@/db";
import { auditLogs, badgeDefinitions, behaviorTemplates, classes, classMemberships, classStudents, guardians, levelDefinitions, organizationMembers, organizations, rewards, schoolYears, studentGuardians, students, users } from "@/db/schema";

export async function getAdminOverview(userId: string, filters: { entityType?: string; action?: string; organizationId?: string } = {}) {
  ...
  const selectedOrganization = requestedOrganizationId
    ? organizationsForUser.find((organization) => organization.id === requestedOrganizationId)
  ...
  const [classRows, memberRows, classAccessRows, auditRows, studentTotal, guardianTotal, behaviorTotal, levelTotal, badgeTotal, rewardTotal, schoolYearRows] = await Promise.all([
    db.select({ id: classes.id, organizationId: classes.organizationId, name: classes.name, grade: classes.grade, schoolYearId: classes.schoolYearId, schoolYearName: schoolYears.name, homeroomTeacherId: classes.homeroomTeacherId, settingsJson: classes.settingsJson, studentCount: sql<number>`count(distinct ${classStudents.studentId})` }).from(classes).innerJoin(schoolYears, eq(schoolYears.id, classes.schoolYearId)).leftJoin(classStudents, and(eq(classStudents.classId, classes.id), sql`${classStudents.leftAt} is null`)).where(inArray(classes.organizationId, organizationIds)).groupBy(classes.id, schoolYears.name).orderBy(asc(classes.name)),
    db.select({ userId: organizationMembers.userId, displayName: users.displayName, email: users.email, status: users.status, organizationId: organizationMembers.organizationId, role: organizationMembers.role }).from(organizationMembers).innerJoin(users, eq(users.id, organizationMembers.userId)).where(and(inArray(organizationMembers.organizationId, organizationIds), inArray(users.status, ["active", "invited"]))).orderBy(asc(users.displayName)),
    db.select({ classId: classMemberships.classId, userId: classMemberships.userId, role: classMemberships.role }).from(classMemberships).innerJoin(classes, eq(classes.id, classMemberships.classId)).innerJoin(users, eq(users.id, classMemberships.userId)).where(and(inArray(classes.organizationId, organizationIds), eq(users.status, "active"))),
  ...
  return { organizations: organizationsForUser, selectedOrganizationId: selectedOrganization.id, classes: classRows, members: memberRows, classAccess: classAccessRows, schoolYears: schoolYearRows, auditLogs: auditRows, studentCount: Number(studentTotal[0]?.total ?? 0), guardianCount: Number(guardianTotal[0]?.total ?? 0), configuration: { behaviors: Number(behaviorTotal[0]?.total ?? 0), levels: Number(levelTotal[0]?.total ?? 0), badges: Number(badgeTotal[0]?.total ?? 0), rewards: Number(rewardTotal[0]?.total ?? 0) }, filters: { entityType, action, organizationId: selectedOrganization.id } };
}
