...
- `REQUIRE_RUNTIME_ROLE=true` là guard fail-closed: authenticated CI và rollout production sẽ không fallback về `DATABASE_URL` nếu thiếu `DATABASE_URL_RUNTIME`. Chỉ bật flag sau khi runtime role đã được tạo và kiểm tra bằng `npm run db:verify`.
